...
//
// M7-T01 lands the skeleton and the greedy branch only. Any parameter whose
// behaviour is not yet implemented (temperature > 0, non-default penalties,
// top-k/top-p, logprobs, stop strings/ids) is rejected up front by
// `Sampler::Create` with `Unimplemented`, so no requested knob is ever silently
// ignored — each later ticket removes its guard as it lands the stage.
...
// A configured, validated sampler for one request's `SamplingParams`. Owns only
// the params (and, from T02, per-request RNG/scratch state) — cheap to hold per
// sequence and safe to reuse across steps of the same request.
class Sampler {
 public:
  ...
  // parameter whose stage M7-T01 has not yet implemented. On success the
  // returned sampler's `Sample` is guaranteed to be within the implemented
  // subset.
  ...
  //
  // T01 implements the greedy branch: argmax with a strict `>` scan so the
  // lowest vocab index wins ties (deterministic — two identical requests are
  // bit-identical). A non-finite maximum (a NaN logit, i.e. an upstream
  // numerics bug) is surfaced as `Internal` rather than returned as a bogus
  // token; an empty `logits` row is `InvalidArgument`.
  // `const` in T01 (greedy is a pure function of the logits); T02 introduces
  // per-request RNG state, at which point the advancing counter becomes a
  // `mutable` member so this stays const, or the qualifier is dropped.
 ...
 private:
  explicit Sampler(SamplingParams params) : params_(std::move(params)) {}
