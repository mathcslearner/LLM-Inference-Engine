...
**This is the second-generation roadmap.** The project began (v1: `ROADMAP.md`) as a
multi-GPU CUDA engine. After Milestone 2 it pivoted to CPU-first: the development
environment has no NVIDIA hardware, and the post-M2 audit demonstrated that CUDA
written blind accumulates unverifiable risk (a compile-breaking bug survived five
"done" tickets because no CUDA compiler ever saw the code). The decision and its
alternatives are recorded in ADR-004 (written in M3-T01). Milestones 0–2 below are
v1 history, preserved verbatim with their completion markers; M3 retires M2's
CUDA-specific deliverables and stands up the CPU compute substrate in their place.
Everything from M3 onward is the CPU-first plan.
...
> as project history; their design doc (`docs/design/cuda-backend.md`) stays in-tree,
> marked retired, as the starting point for any future GPU backend.
...
### M3-T01 · ADR-004: the CPU-first pivot
...
Linux x86-64 CI). Mark `docs/design/cuda-backend.md` as retired (banner note
pointing here — the doc stays as reference for a future GPU backend). Update
`CLAUDE.md` (scope paragraph, module table, build notes) and `README` to the
CPU-first framing; `ROADMAP.md` (v1) gains a banner pointing to this file.
**Depends on:** M2-T09.
**Acceptance criteria:**
...
- CLAUDE.md, README, and the v1 roadmap banner all state the pivot; no doc claims the engine targets NVIDIA GPUs.

### M3-T02 · CUDA excision
